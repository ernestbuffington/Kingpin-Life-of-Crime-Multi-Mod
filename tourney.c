...
char Pistol_Dmg[16];          // kp pistol damage setings
char Blakjack_Dmg[16];        // kp black jack damage setings
char ShotGun_Dmg[16];	      // kp shotgun damage setings
char TommyGun_Dmg[16];        // kp TommyGun damage setings
char HMG_Dmg[16];		      // kp HMG damage setings
char Grenade_Dmg[16];	      // kp grenade launcher damage setings
char Rocket_Dmg[16];	      // kp rocket launcher damage setings
char Flame_thrower_Dmg[16];	  // kp FlameThrower damage setings
char Machinegun_Dmg[16];      // q2 Machinegun damage setings
char Rail_Gun_Dmg[16];		  // q2 RailGun damage setings
char Chain_Gun_Dmg[16];	      // q2 Chain gun damage setings
char Hyper_Blaster_Dmg[16];	  // q2 HyperBlaster damage setings
char BFG10k_Dmg[16];		  // q2 BFG10k damage setings
char Super_Shitgun_Dmg[16];	  // q2 SupershotGun damage setings

// enable weapons
int enable_TommyGun;			 // enable Tommygun
int enable_MHG;		             // enable HMG
int enable_Grenade_Launcher;	 // enable grenade launcher
int enable_Bazooka;				 // enable rocket luncher
int enable_Flame_Thrower;		 // enable flame thrower
int enable_Crowbar;				 // enable crowbar
int enable_Pistol;				 // enable kp pistol
int enable_Shotgun;				 // enable kp shotgun
int enable_Railgun;				 // enable railgun
int enable_Bfg;					 // enable bfg10k
int enable_Machinegun;			 // enalbe machine gun
int enable_Super_Shotgun;		 // enable super shotgun
int enable_Chain_Gun;			 // enable chaingun
int enable_Hyper_Blaster;		 // enable hyperblaster

// enable quaked
int enable_quaked;				 // enable quaked
int enable_shards;				 // enable shards
int enable_combatarmor;			 // enable combat armore
int enable_bodyarmor;			 // enable body armor
int enable_quad;				 // enable quad damage
int enable_invulnerability;		 // enable invulnerability
int enable_quadfire;			 // enable quadfire

// enable converting of weapons
int enable_convertq2weapons;	 // enable converting q2 weapons to kp weapons
int enable_convertq2health;		 // enable converting q2 health to kp health
int enable_convertq2armor;		 // enable converting q2 armor to kp
int enable_convertq2powerups;	 // enable converting q2 poeer ups to kp power ups

int enable_teleports;		     // enable teleports
int enable_hooks;			     // enable hooks
int enable_jetpack;			     // enable jet pack
int enable_cash_snatching;       // Please welcome our own Private Dick, Mr. Cash Snatching
int enable_cash_snatching_bomb;  // Please welcome our own Private Dick, Mr. Cash Snatching
...
edict_t *GetAdmin()
{
	...
	for_each_player_not_bot(doot, i)// KOOGLEBOT_ADD
	{
...
void CheckEndVoteTime () // check the timelimit for voting next level/start next map
{
	...
	for (i = 1; i <= (int)maxclients->value; i++) //hypov8 was macro. would reset on newly connected clients
	{
		...
		if (ent->client->pers.connected && !ent->kooglebot.is_bot)
		{
			count++;
			if (ent->inuse)
