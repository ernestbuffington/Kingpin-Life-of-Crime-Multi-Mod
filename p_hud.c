...
void VoteMapScoreboardMessage (edict_t *ent)
{
	...
	for_each_player_not_bot(player, i)// KOOGLEBOT_ADD
	{
...
// KOOGLEBOT_ADD
	if (ent->kooglebot.is_bot)	return;
// KOOGLEBOT_END
	...
	Com_sprintf(entry, sizeof(entry), "xm -50 yt 25 dmstr 953 \"Skill: %i of 10\" ", KOOGLECM_ReturnBotSkillWeb());
	j = strlen(entry);
	strcpy(string + stringlength, entry);
	stringlength += j;
	...
	for (team=1; team<=2; team++)
	{
		...
		for (i=0 ; i<total ; i++)
		{
			...
			if (cl_ent == ent)
				...
				tag = "779";
// KOOGLEBOT_ADD
			else if (cl_ent->kooglebot.is_bot)
				tag = "966";
// KOOGLEBOT_END
			else
				tag = "999";	// fullbright
