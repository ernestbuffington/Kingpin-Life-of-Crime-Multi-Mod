...
// KOOGLEBOT_ADD
		if (level.bot_debug_mode)
		{
			if (self->kooglebot.is_bot)
			{
				...
				//gi.WritePosition (self->s.origin);
				//gi.WriteDir (self->kooglebot.move_vector);
...
// KOOGLEBOT_ADD
	if (self->kooglebot.is_bot)
		KOOGLEMV_Attack_CalcRandDir(self, aimdir);
// KOOGLEBOT_END
...
void fire_rocket (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius, int radius_damage)
{
	...
	// KOOGLEBOT_ADD
	if (self->kooglebot.is_bot)
		KOOGLEMV_Attack_CalcRandDir(self, dir);
	// KOOGLEBOT_END
...
// KOOGLEBOT_ADD
	//if (self->kooglebot.is_bot)
		//KOOGLEMV_Attack_CalcRandDir(self, forward);
// KOOGLEBOT_END
