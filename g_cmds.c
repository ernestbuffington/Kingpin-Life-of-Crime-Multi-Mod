...
void Cmd_Spec_f (edict_t *self)
{
	if (self->client->pers.spectator == SPECTATING)
		return;
// KOOGLEBOT_ADD
	if (self->kooglebot.is_bot) /* hypov8 added so it dont check bots for idle issues(no nodes etc..) */
		return;
// KOOGLEBOT_END
...
// KOOGLEBOT_ADD //MENU
	if (KOOGLECM_G_SelectNextItem(ent))
		return;
// KOOGLEBOT_END //MENU
...
// KOOGLEBOT_ADD //MENU
	if (KOOGLECM_G_SelectPrevItem(ent))
		return;
// KOOGLEBOT_END //MENU
...
// KOOGLEBOT_ADD
			if (g_edicts[a].kooglebot.is_bot)
				safe_cprintf(ent, PRINT_HIGH, "%3d %5d %4s %-15s %7d %s\n",
					a-1, c->resp.score, buf, c->pers.netname, curtime-c->pers.lastpacket, va("(bot) %1.1fx", g_edicts[a].kooglebot.botSkillMultiplier) );
			else
// KOOGLEBOT_END
				safe_cprintf(ent, PRINT_HIGH, "%3d %5d %4s %-15s %7d %s\n",
	...
	if (sv_bot_allow_skill->value || sv_bot_allow_add->value)
	{
		if (sv_bot_allow_skill->value)
		{
			if (ent->client->pers.admin > NOT_ADMIN)
				...
				strcat(buf, "VoteBotSkill 0-10"); // KOOGLEBOT_ADD
		}
		if (sv_bot_allow_add->value)
			strcat(buf, ", VoteBotRemove x, VoteBotAdd 1=Dragons"); // KOOGLEBOT_ADD
...
// KOOGLEBOT_ADD			
				...
				case	VOTE_ADDBOT:
					if (level.modeset == MATCH || level.modeset == PUBLIC)	
					{
						...
						if (teamplay->value)
							KOOGLESP_SpawnBot_Random(voteAddBot, "\0", "\0", NULL);
						else 		
				...
				case	VOTE_REMOVEBOT:
						if (level.modeset == MATCH || level.modeset == PUBLIC)	
						{
							...
							KOOGLESP_RemoveBot(voteRemoveBot, true);
						}
					break;
				case VOTE_BOTSKILL:
					{
		...
		if ((novn *2) >= nop)
		{
			switch (level.voteset) // Papa - if you wanted to add different types of votes, you could do it here
			{
				...
				case VOTE_ON_MAP:
					...
					break;
// KOOGLEBOT_ADD //hypov8 todo?: check this
				case	VOTE_ADDBOT:
...
// KOOGLEBOT_ADD //hypov8 todo: test this
