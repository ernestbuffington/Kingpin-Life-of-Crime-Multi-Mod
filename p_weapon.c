...
// KOOGLEBOT_ADD
	//if (ent->kooglebot.is_bot)
		//KOOGLEMV_Attack_CalcRandDir(ent, forward);
// KOOGLEBOT_END
...
// KOOGLEBOT_ADD
	...
	if (ent->kooglebot.is_bot)
	{
		...
		if (ent->enemy){
			...
			enOrig[2] += ent->kooglebot.enemyOriginZoffset ;
		}
		else
			VectorCopy(ent->kooglebot.enemyOrigin, enOrig);
...
#if HYPODEBUG
			gi.dprintf(" KOOGLE: Rocket Will Hit Wall\n");
#endif
			return;
		}
	}	
