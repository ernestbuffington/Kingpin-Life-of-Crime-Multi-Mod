...
// Bot Scoreboard
/////////////////////////////////////////////////////////////////////////////////
void KOOGLECM_BotScoreboardVote(edict_t *ent) //SCORE_BOT_MENU
{
	char	string[1024];
	...
	if (sv_bot_allow_skill->value != 0)
		Com_sprintf(skill, sizeof(skill), "%i of 10", KOOGLECM_ReturnBotSkillWeb());//web skill
	else
...
// Bot Scoreboard Add
/////////////////////////////////////////////////////////////////////////////////
void KOOGLECM_BotScoreboardAdd(edict_t *ent) // SCORE_BOT_ADD
{
	char	string[1024];
	char	color[MENU_Y_COUNT][4];
	int		yofs[MENU_Y_COUNT + 3]; //y offset. +3 for headder.
...
void KOOGLECM_BotScoreboardRemove(edict_t *ent) //SCORE_BOT_REMOVE
{
	int outCount;
	edict_t	*bot;
	char	color[MENU_Y_COUNT][4];
...
// Skill Scoreboard
/////////////////////////////////////////////////////////////////////////////////
void KOOGLECM_BotScoreboardSkill(edict_t *ent) //SCORE_BOT_SKIL
{
	char	string[1024];
	char	color[MENU_Y_COUNT][4];
	int		yofs[MENU_Y_COUNT + 3]; //y offset. +3 for headder.
...
static void Cmd_VoteAddBot_f(edict_t *ent, int teamUse)
{
	...
	if (level.voteset == NO_VOTES)
	{
		...
		if (count == 1)
		{
			...
			//KOOGLESP_SpawnBot(team, "\0", "\0", NULL);
			KOOGLESP_SpawnBot_Random(team, "\0", "\0", NULL);
			return;
		}
		...
		if (teamplay->value)
	    strcpy(voteAddBot, s);
		//strcpy_s(s, sizeof(s), voteAddBot);
	...
	if (!usedMenu) 
	{
		...
		if (s[0] == '\0' || skill < 0 || skill > 10)
		{
			gi.cprintf(ent, PRINT_HIGH, "INVALID BOT SKILL. VALUE= 0 to 10.(current: %d)\n",
				ent->client->pers.netname, KOOGLECM_ReturnBotSkillWeb());
			return;
		}
		skill_f = KOOGLECM_ReturnBotSkillFloat(skill);
	}
	else
...
//
/////////////////////////////////////////////////////////////////////////////////
int KOOGLECM_ReturnBotSkillWeb(void)
{
...
//
/////////////////////////////////////////////////////////////////////////////////
int KOOGLECM_ReturnBotSkillWeb_var(float skill)
{
	if (skill < .2) return 0;
	else if (skill < .6) return 1;
	else if (skill < 1.0) return 2;
...
//
/////////////////////////////////////////////////////////////////////////////////
void KOOGLECM_BotDebug(qboolean changeState) //Fix TheGhost
{
	int		i;
	edict_t	*doot;
...
//
/////////////////////////////////////////////////////////////////////////////////
void KOOGLECM_BotAdd(char *name, char *skin, char *team, char *skill)
{
	if (level.bots_spawned &&(level.modeset == MATCH || level.modeset == PUBLIC))
	{
		...
		if (teamplay->value) // name, skin, team 
			KOOGLESP_SpawnBot(team, name, skin, NULL, _skill); //sv addbot thugBot "male_thug/009 031 031" dragon 1.0
		else // name, skin			
			KOOGLESP_SpawnBot("\0", name, skin, NULL, _skill); //sv addbot thugBot "male_thug/009 031 031" null 1.0
	}
...
//
/////////////////////////////////////////////////////////////////////////////////
void KOOGLECM_Menu_f(edict_t *ent)
{
...
//
/////////////////////////////////////////////////////////////////////////////////
void KOOGLECM_SetBotSkill_f(edict_t *ent, char *value) //add hypov8
{
...
qboolean KOOGLECM_Commands(edict_t *ent)
{
	...
	if (Q_stricmp(cmd, "addnode") == 0)
	{
		...
		ent->kooglebot.pm_last_node = KOOGLEND_AddNode(ent, atoi(gi.argv(1)), false);
		if (!sv_botpath->value)
	...
	if(Q_stricmp (cmd, "findnode") == 0)
	{
		...
		node = KOOGLEND_FindClosestNode(ent,BOTNODE_DENSITY, BOTNODE_ALL);
		if (node != INVALID){
			if (dedicated->value)
		...
		KOOGLEND_ShowNode(node, 1); //hypov8 show closest node
...
{
	int     i;
	char	bigbuffer[MAX_BUFFER_SIZE];
	int		len;
	va_list	argptr;
	edict_t	*cl_ent;
	...
	{
		cl_ent = g_edicts + 1 + i;
		if (!cl_ent->inuse || cl_ent->kooglebot.is_bot)
			continue;
...
void safe_cprintf (edict_t *ent, int printlevel, char *fmt, ...)
{
	...
	if (ent && (!ent->inuse || ent->kooglebot.is_bot))
		return;
...
void safe_centerprintf (edict_t *ent, char *fmt, ...)
{
	...
	if (!ent->inuse || ent->kooglebot.is_bot)
		return;
