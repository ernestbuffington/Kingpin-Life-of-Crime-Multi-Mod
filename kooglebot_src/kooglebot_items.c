...
qboolean KOOGLEIT_CheckIfItemExists(edict_t *self)
{
	...
	if (self->kooglebot.node_goal == INVALID)
		return true;
	if (nodes[self->kooglebot.node_goal].type != BOTNODE_ITEM)
		return true;
	if (self->kooglebot.targetPlayerNode > level.framenum)
		return true; //hypov8 player.

	//add hypov8 chect if item is still valid
	for (i = 0; i < num_items; i++)	{
		if (item_table[i].node == self->kooglebot.node_goal)
		{
...
void KOOGLEIT_PlayerRemoved(edict_t *ent)
{
	if (!ent->kooglebot.is_bot && level.bots_spawned && sv_bot_max_players->value && level.botsRemoved)
	{
		...
		if (count < (int)sv_bot_max_players->value && num_bots < sv_bot_max->value)
		{
			...
			KOOGLESP_SpawnBot_Random('\0', "\0", "\0", NULL);
			level.botsRemoved--;
		}
	}
}
		...
		if (dist <128) //jump?
		{
			...
			if (trace.allsolid == 0 && trace.startsolid == 0 && trace.fraction == 1.0)
			{
				if (dist <= 64){
					...
					//self->kooglebot.canJump_itemID= 
					//self->kooglebot.isJumpToCrate = true;
					//self->kooglebot.crate_time = level.framenum + 3;
				}
				return true;
			}
		}
...
qboolean KOOGLEIT_ChangeWeapon (edict_t *ent, gitem_t *item)
{
	...
	if (item->ammo)
	{
		...
		clip_index = KOOGLEIT_ClipNameIndex(item);
...
float KOOGLEIT_ItemNeed(edict_t *self, int item, float timestamp, int spawnflags)
{
	...
	int nedWepMulti = (self->kooglebot.num_weps <= 4) ? 2.0 : 1.0;
...
// END
	...
	switch (item)
	{
		...
		// Armor
		case ITEMLIST_ARMORHELMET:			if (KOOGLEIT_Needs_Armor(self, "item_armor_helmet"))			return  0.4; break;
		case ITEMLIST_ARMORJACKET:			if (KOOGLEIT_Needs_Armor(self, "item_armor_jacket"))			return 0.4;	break;
		case ITEMLIST_ARMORLEGS:			if (KOOGLEIT_Needs_Armor(self, "item_armor_legs"))				return 0.4;	break;

		case ITEMLIST_ARMORHELMETHEAVY:		if (KOOGLEIT_Needs_Armor(self, "item_armor_helmet_heavy"))		return 0.8;	break;
		case ITEMLIST_ARMORJACKETHEAVY:		if (KOOGLEIT_Needs_Armor(self, "item_armor_jacket_heavy"))		return 0.8;	break;
		case ITEMLIST_ARMORLEGSHEAVY:		if (KOOGLEIT_Needs_Armor(self, "item_armor_legs_heavy"))		return 0.8;	break;
...
float KOOGLEIT_ItemNeedSpawned(edict_t *self, int item, float timestamp, int spawnflags)
{
	//gitem_t *itemArmor;
...
void KOOGLEIT_BuildItemNodeTable(qboolean reLinkEnts)
{
	...
	for (items = g_edicts; items < &g_edicts[globals.num_edicts]; items++)
	{
		...
		// Items
		/////////////////////////////////////////////////////////////////
		item_index = KOOGLEIT_ClassnameToIndex(items->classname, items->style); //hypov8 add safe styles
		...
		if (strcmp(items->classname, "func_plat") == 0)
		{
			if (!reLinkEnts)
			{
				item_table[num_items].node = KOOGLEND_AddNode(items, BOTNODE_PLATFORM, true);
		...
		if (strcmp(items->classname, "misc_teleporter") == 0)
		{
			if (!reLinkEnts)
			{
				item_table[num_items].node = KOOGLEND_AddNode(items, BOTNODE_TELEPORTER, true);
				item_table[num_items].item = 99;
				num_items++;
				continue; //add hypov8
			}
		...
		{
			if (!reLinkEnts)
				KOOGLEND_AddNode(items, BOTNODE_TRIGPUSH, true);
		...
		if (!reLinkEnts)
		{
			...
			item_table[num_items].node = KOOGLEND_AddNode(items, BOTNODE_ITEM, true);
			num_items++;
		}
		else // Now if rebuilding, just relink ent structures 
		{
			...
			for (i = 0; i < numnodes; i++)
			{
					...
					|| nodes[i].type == BOTNODE_PLATFORM)
				{
					...
					v[2] += KOOGLEND_NodeOffset(nodes[i].type);
			...
			if (i == numnodes)
			{
				...
				{	
					item_table[num_items].node = KOOGLEND_AddNode(items, BOTNODE_ITEM, true);//hypov8 todo: should we add a node
					num_items++;
				}
#if HYPODEBUG //allways show in my debug ver
				gi.dprintf(" KOOGLE: WARNING: Relink item:%s pos:(%1.0f %1.0f %1.0f) match_XY:%s\n",
