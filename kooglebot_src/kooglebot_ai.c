...
//hypov8 match KOOGLEAI_VisibleEnemy(). except for glass/fence
///////////////////////////////////////////////////////////////////////
static qboolean KOOGLEAI_CheckNonLeadShot(edict_t *self, qboolean isRocket)
{
...
static void KOOGLEAI_ChooseWeapon(edict_t *self)
{
	...
	{
		//HMG.
		if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(0,1))))
			return;
		//RL.
		if (KOOGLEAI_CheckNonLeadShot(self, true)  //check if fence infront
			&& KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(0,2))))
			return;
		...
		if (range > 100 && range < 920 && (self->enemy->s.origin[2] - 20) < self->s.origin[2])
			if (KOOGLEAI_CheckNonLeadShot(self, false)  //check if fence infront
				&& KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(0,3))))
				return;
		//Flamer.
		if (range < 720)// Flamethrower.
			if (KOOGLEAI_CheckNonLeadShot(self, false)  //check if fence infront
				&& KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(0,4))))
				return;
		//Tommy.
		if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(0,5))))
			return;
	}
	break;
	...
	case 1:
	{
		...
		if (KOOGLEAI_CheckNonLeadShot(self, true)  //check if fence infront
			&& KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(1,1))))
			return;
		//HMG.
		if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(1,2))))
			return;
		//Tommy.
		if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(1,3))))
			return;

		//GL. Only use in certain ranges and only on targets at or below our level
		if (range > 100 && range < 920 && (self->enemy->s.origin[2] - 20) < self->s.origin[2])
			if (KOOGLEAI_CheckNonLeadShot(self, false)  //check if fence infront
				&& KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(1,4))))
				return;
		//Flamer.
		if (range < 720)
			if (KOOGLEAI_CheckNonLeadShot(self, false)  //check if fence infront
				&& KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(1,5))))
				return;
	}
	break;
	...
	{
		//Tommy.
		if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(2,1))))
			return;
		//HMG.
		if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(2,2))))
			return;
		//RL.
		/*if (range > 80)*/// Longer range 
		if (KOOGLEAI_CheckNonLeadShot(self, false)  //check if fence infront
			&& KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(2,3))))
			return;
		//GL. Only use in certain ranges and only on targets at or below our level
		if (range > 100 && range < 920 && (self->enemy->s.origin[2] - 20) < self->s.origin[2])
			if (KOOGLEAI_CheckNonLeadShot(self, false)  //check if fence infront
				&& KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(2,4))))
				return;
		//Flamer.
		if (range < 720)
			if (KOOGLEAI_CheckNonLeadShot(self, false)  //check if fence infront
				&& KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(2,5))))
				return;
	}
	break;
	}//end switch
	...
	if (KOOGLEIT_ChangeWeapon(self, FindItem("Shotgun")))
		return;
	if (KOOGLEIT_ChangeWeapon(self, FindItem("SPistol")))
		return;
	...
	if (KOOGLEAI_CheckNonLeadShot(self, false)  //check if fence infront
		&& KOOGLEIT_ChangeWeapon(self, FindItem("Crowbar")))
		return;

	if (/*KOOGLEAI_CheckNonLeadShot(self)  //check if fence infront
		&& */KOOGLEIT_ChangeWeapon(self, FindItem("Pipe"))) //hypo last resort no checks
		return;
...
static void KOOGLEAI_ChooseWeaponHM(edict_t *self)
{
	...
	default:
	case 1: // "pistol"
		KOOGLEIT_ChangeWeapon(self, FindItem("Pistol"));			break;
	case 2: // "shotgun"
		KOOGLEIT_ChangeWeapon(self, FindItem("Shotgun"));			break;
	case 3: // "tommygun"
		KOOGLEIT_ChangeWeapon(self, FindItem("Tommygun"));			break;
	case 4: // "Heavy machinegun"
		KOOGLEIT_ChangeWeapon(self, FindItem("Heavy machinegun")); break;
	case 5: // "Grenade Launcher"
		KOOGLEIT_ChangeWeapon(self, FindItem("Grenade Launcher"));	break;
	case 6: // "Bazooka"
		KOOGLEIT_ChangeWeapon(self, FindItem("Bazooka")); break;
	case 7: // "FlameThrower"
		KOOGLEIT_ChangeWeapon(self, FindItem("FlameThrower"));		break;
	}
}
...
static void KOOGLEAI_PickShortRangeGoal(edict_t *self)
{
	...
	{
		self->kooglebot.SRGoal_onLadder = level.framenum + 10;
		self->movetarget = NULL;
		return;
	}
	if (self->kooglebot.SRGoal_onLadder > level.framenum)
		return;

	if (self->groundentity == NULL && !self->kooglebot.isJumpToCrate)
		return;
	...
	if (!(int)sv_hitmen->value && 
		(self->kooglebot.num_weps <= 2|| (self->client->pers.weapon && !strcmp(self->client->pers.weapon->classname, "weapon_pistol"))))
	{
		if (self->kooglebot.SRGoal_frameNum < level.framenum)
			lookDist = 512;
		if (!(level.framenum % 30))
			self->kooglebot.SRGoal_frameNum = level.framenum + 30;
	}
	...
	while (target)
	{
		...
		{
			if (KOOGLEAI_InfrontBot(self, target))
			{
		...
		if (!isRocket)
		{
			...
			if (KOOGLEIT_IsReachable(self, itemOrigin))
			{
				if (KOOGLEAI_InfrontBot(self, target)) //look 360 deg for weapons
				{
					float dist = VectorDistance(target->s.origin, self->s.origin);
					index = KOOGLEIT_ClassnameToIndex(target->classname, target->style); //hypov8 add safe styles
					weight = KOOGLEIT_ItemNeed(self, index, target->timestamp, target->spawnflags);
...
qboolean KOOGLEAI_PickShortRangeGoalSpawned(edict_t *self)
{
	...
	while (target)
	{
		...
		if (KOOGLEIT_IsReachable(self, target->s.origin) && target->solid != SOLID_NOT)
		{		
			if (strncmp(target->classname, "w", 1) == 0)
			{
				index = KOOGLEIT_ClassnameToIndex(target->classname, target->style); //hypov8 add safe styles
				weight = KOOGLEIT_ItemNeedSpawned(self, index, target->timestamp, target->spawnflags);
...
static qboolean KOOGLEAI_WeaponCount(edict_t *self)
{
	...
	if (wepCount > self->kooglebot.num_weps)
	{
		self->kooglebot.num_weps = wepCount;
		return true;
	}
	return false;
}
...
qboolean KOOGLEAI_PickShortRangeGoal_Player (edict_t *self, qboolean reCheck)
{
	...
	if (self->kooglebot.botSkillDeleyTimer > level.time)
		return false;
	if (!self->kooglebot.enemyID)
		return false;
	//no longer intrested in enemy
	if (self->kooglebot.enemyAddFrame <= level.framenum - 150)
		return false;

	//check our enemy. again?
	if (reCheck || self->kooglebot.enemyChaseFrame <= level.framenum)
	{
		if (random() > 0.5/*&&self->kooglebot.isChasingEnemy*/)
		{
			self->kooglebot.isChasingEnemy = false;
			self->kooglebot.enemyID = -1;
			self->kooglebot.enemyAddFrame = 0;
			KOOGLEAI_PickLongRangeGoal(self);
			return false;
		}
		else
		{
			...
			for (i = 1; i <= (int)maxclients->value; i++)
			{
				...
				if (!players->kooglebot.is_validTarget)
					continue;
				...
				if (i != self->kooglebot.enemyID)
					continue;

				node = KOOGLEND_FindClosestNode(players, BOTNODE_DENSITY, BOTNODE_ALL);
				if (node == INVALID)
				{
					KOOGLEAI_Reset_Goal_Node(self, 1.0, "No close node for SRG (Player).");
					return false;
				}
				...
				{
					KOOGLEAI_Reset_Goal_Node(self, 1.0, "No close node for SRG (Bot).");
					return false;
				}

				// OK, everything valid, let's start moving to our goal.
				self->kooglebot.targetPlayerNode = level.framenum + 50; //add hypov8 +3 sec timeout
				self->kooglebot.node_current = current_node;
				...
				self->kooglebot.state = BOTSTATE_MOVE;
				self->kooglebot.node_tries = 0; // Reset the count of how many times we tried this goal
				if (!self->kooglebot.isChasingEnemy)
					self->kooglebot.enemyChaseFrame = level.framenum + 80;
				self->kooglebot.isChasingEnemy = true;
				self->kooglebot.node_goal = node;
...
void KOOGLEAI_PickLongRangeGoal(edict_t *self)
{
	...
	if(current_node == INVALID)
	{
		KOOGLEAI_Reset_Goal_Node(self, 1.0, "No close node for LRG.");
		return;
	}
	...
	for(i=0;i<num_items;i++)
	{
		...
		weight = KOOGLEIT_ItemNeed(self, item_table[i].item, 0.0, 0);
		if (weight <= 0.0) 
			continue;
	...
	self->kooglebot.targetPlayerNode = 0; //add hypov8
	///////////////////////////////////////////////////////
	// Players
	///////////////////////////////////////////////////////
	...
	for (i = 1; i <= (int)maxclients->value; i++)
	{
		...
		{
			weight = 2.5;
			if (players->kooglebot.is_hunted){
		...
		{
			weight = 0.3;
			if (players->kooglebot.is_hunted && self->health>80 && self->kooglebot.num_weps > 3){
				cost /= 2; //perswade some more
				weight = 6; //lets go hunting
			}
		}
		...
		if(weight > best_weight)
		{	
			self->kooglebot.targetPlayerNode = level.framenum + 100; //add hypov8 +10 sec timeout
			best_weight = weight;
			goal_node = node;
			goal_ent = players;
		}	
	...
	{
		KOOGLEAI_Reset_Goal_Node(self, 1.0, "Did not find a LR goal");
		return; // no path? 
	}
	...
	KOOGLEND_SetGoal(self,goal_node, goal_ent);
...
static qboolean KOOGLEAI_VisibleEnemy(edict_t *self, edict_t *other, qboolean isBestplayer)
{
	...
	self->kooglebot.aimLegs = 0;
	self->kooglebot.enemyOriginZoffset = 0;
	...
	if (isBullet)
	{
		...
		else if (self->kooglebot.botSkillCalculated < 3){
...
static void KOOGLEAI_CalculatePlayerState()
{
	...
	for (i = 1; i <= (int)maxclients->value; i++)
	{
		...
		players->kooglebot.is_validTarget = false;
		players->kooglebot.is_hunted = false;
		...
		if (players->kooglebot.is_bot)
		{
			num_bots++; //allow calculation at vote
			players->kooglebot.botSkillCalculated = KOOGLEAI_SkillMP(players->kooglebot.botSkillMultiplier, skill);
		}
		...
		if (players->client->invincible_framenum > level.framenum)	{
			players->kooglebot.pm_last_node = INVALID;	//is_validTarget = false;
		}
		else // route/attack this player		
			players->kooglebot.is_validTarget = true;	
	...
	if (total >1 && count >1)
	{
		...
		if (score > 3 && !g_edicts[botRankPlayerNum[0]].kooglebot.is_bot)
			(g_edicts + botRankPlayerNum[0])->kooglebot.is_hunted = true;
	}
}
#if HYPOBOTS
static qboolean KOOGLEAI_PickOnBestPlayer(int playerNum)
{
	int tmpScore;
...
void KOOGLEAI_G_RunFrame(void)
{
	...
	KOOGLEAI_CalculatePlayerState(); //set hunted, player/bot count etc..
	...
	if (level.bots_spawned && (level.modeset == MATCH || level.modeset == PUBLIC))
	{
		...
		for (i = 1; i <= (int)maxclients->value; i++, ent++)
		{
			...
			{
				ent->kooglebot.PM_firstPlayer = false;
				if (!foundFirstPlyr) //auto route on player 1
				{
					if (KOOGLEND_PathMapValidPlayer(ent))
					{
						if ((int)teamplay->value != 1) //route all players in teamplay
							foundFirstPlyr = true;
						ent->kooglebot.PM_firstPlayer = true;
						KOOGLEND_PathMap(ent, false); //run every 100ms. jump is caught in p_client
					}
					else
...
{
	vec3_t	vec;
	float	dot, fov, skill = self->kooglebot.botSkillCalculated;
	vec3_t	forward;
...
static qboolean KOOGLEAI_FindEnemy(edict_t *self)
{
	...
	if (self->kooglebot.isOnLadder)//  == 1) //allow attackt at top of ladder??
		return false;
	...
	if (self->kooglebot.lastDamageTimer >= (level.framenum - 40)) //4 seconds. allow for skill
		underAttack = true;
	 ...
	 for (i = 1; i <= (int) maxclients->value; i++) //num_players
	 {
		 ...
		 if (self->kooglebot.enemyID == i && players->health < 1)		{
			 self->kooglebot.enemyID = -1;
			 self->enemy = NULL;
			 continue;
		 }
		 ...
		 if (players == self
			 ...
			 || KOOGLEAI_Enemy_ToFarHM(self, players)) // BEGIN HITMEN
			 continue;

		// player is in direct sight. no solid walls
		 if (!players->deadflag && KOOGLEAI_VisibleEnemyPVS(self, players))
		{
			qboolean visable = KOOGLEAI_VisibleEnemy(self, players, players->kooglebot.is_hunted);

			if ((int)sv_bot_hunt->value && players->kooglebot.is_bot == 0 	
				&& players->kooglebot.is_hunted //hypov8 add. stop top player with force!! :)
				&& visable)
			{
				self->enemy = players;
				self->kooglebot.botSkillDeleyTimer = 0;
				return true;
			}
			...
			if (!players->kooglebot.is_hunted && !underAttack && !KOOGLEAI_InfrontEnemy(self, players))
				continue;
	...
	if (j != -1)
	{
		self->kooglebot.enemyID = j;
		self->kooglebot.enemyAddFrame = level.framenum;

		if (self->client->weaponstate == WEAPON_FIRING)
			self->kooglebot.botSkillDeleyTimer = level.time + ((4.0f - self->kooglebot.botSkillCalculated) *0.35f); //hypov8 target changed. shoot next closer player sooner
		else if (underAttack)
			self->kooglebot.botSkillDeleyTimer = level.time + ((4.0f - self->kooglebot.botSkillCalculated) *0.5f); //hypov8 attacked. shoot faster. 2 sec max
		else
			self->kooglebot.botSkillDeleyTimer = level.time + ((4.0f - self->kooglebot.botSkillCalculated) *0.75f); //give player 3 seconds(default) leway between seeing n being shot
	...
	case 0:
		{
			...
			if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(0,1))))
				return;
			else if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(0,2))))
				return;
			else if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(0,3))))
				return;
			else if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(0,4))))
				return;
			else if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(0,5))))
				return;
		}
		break;
	case 1:
		{
			...
			if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(1,1))))
				return;
			else if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(1,2))))
				return;
			else if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(1,3))))
				return;
			else if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(1,4))))
				return;
			else if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(1,5))))
				return;
		}
		break;
	case 2:
	default:
		{
			...
			if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(2,1))))
				return;
			else if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(2,2))))
				return;
			else if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(2,3))))
				return;
			else if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(2,4))))
				return;
			else if (KOOGLEIT_ChangeWeapon(self, FindItem(KOOGLEAI_Randwep(2,5))))
				return;
		}
		break;
	} //end switch
...
{
	if (sv_hitmen->value) /*enable_hitmen*/
		KOOGLEAI_ChooseWeaponHM(self);//select wep if it has ammo
	else if (KOOGLEAI_WeaponCount(self))
...
void KOOGLEAI_Reset_Goal_Node(edict_t *self, float wanderTime, char* eventName)
{
	...
	{
		self->kooglebot.node_ent = NULL;
		self->kooglebot.node_next = INVALID;
		self->kooglebot.node_goal = INVALID;
		self->kooglebot.state = BOTSTATE_WANDER;
		self->kooglebot.wander_timeout = level.time + wanderTime;
		if (eventName[0])
		{
...
void KOOGLEAI_Think(edict_t *self)
{
	...
	// reset some values
	////////////////////
	VectorCopy(self->s.angles, self->kooglebot.deathAngles);
	...
	if (self->kooglebot.isMovingUpPushed)	{ 
		self->kooglebot.next_move_time = level.time + 0.1f; //stops wander
		if (self->groundentity && self->kooglebot.trigPushTimer < level.framenum){ //allow some time for lunch	
			self->kooglebot.isMovingUpPushed = false;
			self->kooglebot.wander_timeout = level.time -0.1f;
		}
	}
	if (self->kooglebot.isJumpToCrate)	{
		if (level.framenum > self->kooglebot.crate_time || self->groundentity)
			self->kooglebot.isJumpToCrate = false;
	}
	if (self->kooglebot.is_Jumping)
	{
		if (self->groundentity)
			self->kooglebot.is_Jumping = false;
	}


	if (self->kooglebot.water_time >= level.framenum){
		if (!self->groundentity){
			...
			self->kooglebot.water_time = 0;
	}
	if (self->kooglebot.isOnLadder)
	{
		if (self->kooglebot.ladder_time < level.framenum || (self->groundentity && !self->groundentity->client))
		{
			self->kooglebot.ladder_time = 0;
			self->kooglebot.isOnLadder = false; //hypo stop bot attacking on ladders
		}
	}
	...
	/////////////////////////////////////////////
	if (self->kooglebot.isChasingEnemy){
		if (self->kooglebot.enemyChaseFrame < level.framenum){
			KOOGLEAI_Reset_Goal_Node(self, 1.0, "Stoped looking for Enemy.");
			self->kooglebot.isChasingEnemy = false;
			self->kooglebot.enemyID = -1;
			self->enemy = NULL;
		}
	}
	if (self->kooglebot.enemyAddFrame < level.framenum - 150){ //15 seconds, forget about old target
		self->kooglebot.enemyID = -1;
		self->enemy = NULL;
	}
	...
	/////////////////////////////////////////////
	if (self->kooglebot.state == BOTSTATE_WANDER && self->kooglebot.wander_timeout < level.time
	&& !self->kooglebot.isMovingUpPushed && !self->kooglebot.isChasingEnemy)
		KOOGLEAI_PickLongRangeGoal(self);

	BOT_TIME_DIFF(base, timediff, "LRG" , self->kooglebot.state);

	/////////////////////////////////////////////
	// check if bot is moving?
	/////////////////////////////////////////////
	self->kooglebot.moveDirVel = VectorDistance(self->kooglebot.oldOrigin, self->s.origin);
	if (self->kooglebot.moveDirVel > 2)
		self->kooglebot.suicide_timeout = level.time + 10.0;

	tmpTimeout = self->kooglebot.suicide_timeout - level.time;
	if (tmpTimeout < 6 && self->kooglebot.isOnLadder){
		KOOGLEAI_Reset_Goal_Node(self, 1.0, "Stuck on ladder, Turn 180.");
		self->kooglebot.isOnLadder = false;
	...
	////////////////////////////
	if (!self->kooglebot.isMovingUpPushed && !self->client->hookstate)
	{
		KOOGLEAI_PickShortRangeGoal(self);
		BOT_TIME_DIFF(base, timediff, "SRG" , self->kooglebot.state);
	}
	...
	if (KOOGLEAI_FindEnemy(self))
	{
		...
		BOT_TIME_DIFF(base, timediff, "Enemy" , self->kooglebot.state);
		KOOGLEAI_ChooseWeapon(self);
		BOT_TIME_DIFF(base, timediff, "Choose Wep" , self->kooglebot.state);
		KOOGLEMV_Attack(self, &ucmd);
		BOT_TIME_DIFF(base, timediff, "Atack" , self->kooglebot.state);
	}
	else 		// Execute the move, or wander
	{
		KOOGLEAI_PickShortRangeGoal_Player(self, false); 	// keep chasing enemy?
		BOT_TIME_DIFF(base, timediff, "SRG Player", self->kooglebot.state);

		if (self->kooglebot.state == BOTSTATE_WANDER)	{
			KOOGLEMV_Wander(self, &ucmd);
			BOT_TIME_DIFF(base, timediff, "Wander", self->kooglebot.state);
		}
		else if (self->kooglebot.state == BOTSTATE_MOVE)	{
			KOOGLEMV_Move(self, &ucmd);
			BOT_TIME_DIFF(base, timediff, "Move", self->kooglebot.state);
		}
	}
	...
	VectorCopy(self->s.origin, self->kooglebot.oldOrigin);	//hypo set old origin for movement calculations
	...
	if (self->kooglebot.isMovingUpPushed) //stop bot moving on jumppad
	...
	BOT_TIME_DIFF(base, timediff, "END", self->kooglebot.state);
}
