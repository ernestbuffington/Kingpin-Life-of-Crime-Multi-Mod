...
// true.
///////////////////////////////////////////////////////////////////////
static qboolean KOOGLEMV_SpecialMove(edict_t *self, usercmd_t *ucmd)
{
//	int isStep;
	...
	if (self->kooglebot.dodge_time >= level.framenum)
	{

		if (self->kooglebot.dodge_dir == MOVE_LEFT && KOOGLEMV_CanMove_Simple(self, MOVE_LEFT))
		{
		...
		if (self->kooglebot.dodge_dir == MOVE_RIGHT && KOOGLEMV_CanMove_Simple(self, MOVE_RIGHT))
		{
			ucmd->forwardmove = BOT_FORWARD_VEL;
			ucmd->sidemove = BOT_SIDE_VEL;
			//KOOGLEMV_ChangeBotAngle(self);
		...
		VectorSubtract(nodes[self->kooglebot.node_next].origin, self->s.origin, self->kooglebot.move_vector);
		return true;
	}

	//add hypov8 check node ontop of crates
	if (self->kooglebot.node_next > INVALID && self->kooglebot.state == BOTSTATE_MOVE)
	{
		vec_t jump_height = (nodes[self->kooglebot.node_next].origin[2] - 8) - self->s.origin[2];
		...
		if (jump_height > 16 && jump_height <= 60)
		{
			...
			player_move_up[2] = nodes[self->kooglebot.node_next].origin[2];

			distToTarget = VectorDistance(player_move_up, nodes[self->kooglebot.node_next].origin);
			if (distToTarget < 64)
			{
				trace_t trace;
				//hypo can we jump up? minus 12, jump to 60 units hypo: todo
				trace = gi.trace(player_move_up, self->mins, self->maxs, nodes[self->kooglebot.node_next].origin, self, MASK_BOT_SOLID_FENCE);

				if (trace.allsolid == 0 && trace.startsolid == 0 && trace.fraction == 1.0)
				{
					ucmd->forwardmove = BOT_FORWARD_VEL;
					ucmd->upmove = BOT_JUMP_VEL;
					self->kooglebot.isJumpToCrate= true;
					self->kooglebot.crate_time = level.framenum + 5;
	...
	if ((tr_monster.contents & CONTENTS_MONSTER 
		...
		/*&& self->kooglebot.dodge_time < level.framenum*/ )
		{
			int i;
			vec3_t  angles2;
			...
			if (KOOGLEMV_CanMove_Simple(self, MOVE_LEFT))	canGoLeft = true;
			if (KOOGLEMV_CanMove_Simple(self, MOVE_RIGHT))	canGoRight = true;
			...
			for (i = 0; i<2; i++)
			{
				...
				if (i == 0)
				{
					self->kooglebot.dodge_time = level.framenum+5;
					self->kooglebot.dodge_dir = MOVE_LEFT;
					self->s.angles[1] += 45;
					ucmd->forwardmove = BOT_FORWARD_VEL;
					ucmd->sidemove = -BOT_SIDE_VEL;
					KOOGLEMV_ChangeBotAngle(self);
				...
				{
					self->kooglebot.dodge_time = level.framenum +5;
					self->kooglebot.dodge_dir = MOVE_RIGHT;
					self->s.angles[1] -= 45;
					ucmd->forwardmove = BOT_FORWARD_VEL;
					ucmd->sidemove = BOT_SIDE_VEL;
					KOOGLEMV_ChangeBotAngle(self);
		...
		// Check for ladder    //
		////////////////////////
	if (tr.fraction !=1 && tr.contents& CONTENTS_LADDER && KOOGLEMV_CheckLadder(self, ucmd, false, true))
		return true;
	...
	{	
		...
		for (i = 0; i < 4; i++)
		{
			...
			if (/*!tr.allsolid*/ tr.fraction == 1.0)
			{
				...
				self->kooglebot.isJumpToCrate = true;
				self->kooglebot.crate_time = level.framenum + crateT[i];
				return true;
			}
		}
...
static void KOOGLEMV_ChangeBotAngle(edict_t *ent)
{
	...
	VectorNormalize(ent->kooglebot.move_vector);

	current_yaw = anglemod(ent->s.angles[YAW]);
	current_pitch = anglemod(ent->s.angles[PITCH]);

	vectoangles(ent->kooglebot.move_vector, ideal_angle);
...
static int KOOGLEMV_CheckLavaAndSky(edict_t *self) //add normal falling edges
{
	...
	{
		if (KOOGLEMV_CanMove_Simple(self, MOVE_LEFT)){
			self->s.angles[YAW] += 90;
			return true; //return resolved move
		}
		else if (KOOGLEMV_CanMove_Simple(self, MOVE_RIGHT)){
			self->s.angles[YAW] -= 90;
			return true; //return resolved move
		}
		else if (KOOGLEMV_CanMove_Simple(self, MOVE_BACK)){
	...
	else if ((trace.surface->flags & SURF_SKY) && !trace.startsolid)
	{
		...
		else if (KOOGLEMV_CanMove_Simple(self, MOVE_BACK)){ //add hypo fail if all sky
			self->s.angles[YAW] -= 180;
			return true; //return resolved move
		}
		return 2; //cant move
...
///////////////////////////////////////////////////////////////////////
static qboolean KOOGLEMV_CheckEyes_Wander(edict_t *self, usercmd_t *ucmd)
{
	vec3_t  forward, right;
	...
	if (!self->movetarget && self->kooglebot.uTurnTime < level.framenum)
		...
		/*&& self->kooglebot.dodge_time < level.framenum*/)
	{
		if (tr_monster.ent->client && tr_monster.ent->client->ps.pmove.pm_type == PM_NORMAL) //hypo make sure there not in noclip/dead etc
		{
			...
			{
				VectorCopy(self->s.angles, angles2);
				if (i == 0 && KOOGLEMV_CanMove_Simple(self, MOVE_LEFT))
					angles2[1] += 90;//MOVE_LEFT
				else
					angles2[1] -= 90; //MOVE_RIGHT
				...
				if (tr_sides.fraction == 1)
				{
					if (i == 0)
					{
						if (self->kooglebot.dodge_time < level.framenum)
						{
							self->kooglebot.dodge_time = level.framenum + 2;
							self->s.angles[1] += 45;
							ucmd->sidemove = -BOT_SIDE_VEL;
						}
						ucmd->forwardmove = BOT_FORWARD_VEL;
	...
	{
		 if (KOOGLEMV_CheckLavaAndSky(self) == 2) //add hypov8
		...
		&& ((offset[0] == 200 && traceFront.fraction < 0.2) || (offset[0] == 36 && traceFront.fraction < 0.5)))
	{
		...
		if (random() >= 0.8)
		{
			...
			{
				self->kooglebot.water_time = level.framenum + 20;
				return true;
			 }
		}
	...
	if ((traceRight.fraction != 1 || traceLeft.fraction != 1 || isPlayerInfront)
		...
		&& !self->kooglebot.isJumpToCrate)
	{
		trace_t traceUp;

		// Ladder code
		if (traceFront.fraction != 1 && traceFront.contents & CONTENTS_LADDER && KOOGLEMV_CheckLadder(self, ucmd, false, true))
			return true;
		...
		if (traceUp.fraction != 1 || traceUp.allsolid || traceUp.startsolid)
		{
			...
			if (self->kooglebot.uTurnTime < level.framenum)
				self->kooglebot.uTurnCount = 0;
			...
			if (traceRight.fraction > traceLeft.fraction)
			{
				...
				if (self->kooglebot.uTurnTime > level.framenum)
					self->kooglebot.uTurnCount += 1;
				self->kooglebot.uTurnTime = level.framenum + 2;
			}
			else
			{
				//turn anticlockwise
			...
			if (self->kooglebot.uTurnCount > 20)
			{
				...
				self->kooglebot.uTurnTime = 0;
			}
			if (!KOOGLEMV_CheckLavaAndSky(self)) //add hypov8 //todo: check this
				ucmd->forwardmove = BOT_FORWARD_VEL;
			...
			G_ProjectSource(self->s.origin, offset, forward, right, focalpoint);
			VectorSubtract(focalpoint, self->s.origin, self->kooglebot.move_vector);

			return true;
		}
	}	
	if (KOOGLEMV_CheckLavaAndSky(self)== 1)
...
/*
void KOOGLE_Look_Straight(vec3_t player, vec3_t target, vec3_t out)
{
	vec3_t out;
	out[0] = target[0];
	out[1] = target[1];
...
static qboolean KOOGLEMV_MoveDodgeRocket(edict_t *self, usercmd_t *ucmd, qboolean targetAquired)
{
	...
	{
		//look at player
		if (targetAquired || KOOGLEMV_InfrontRocket(self, self->movetarget->owner))
		{	
			...
			if (!targetAquired){
				...
				VectorSubtract(self->movetarget->owner->s.origin, self->s.origin, self->kooglebot.move_vector);
				KOOGLEMV_ChangeBotAngle(self);
			}
			angleDif = playerAngles[1] - rocketAngles[1];
			if (angleDif < 0.0 && angleDif > -180)// rocketAngles[1] > playerAngles[1])
			{
				if (KOOGLEMV_CanMove(self, MOVE_RIGHT) && KOOGLEMV_CanMove_Simple(self, MOVE_RIGHT)){
					ucmd->sidemove = BOT_SIDE_VEL;
					self->kooglebot.last_strafeDir = MOVE_RIGHT;
					self->kooglebot.last_strafeTime = level.framenum + 5;
				}
			}
			else
			{
				if (KOOGLEMV_CanMove(self, MOVE_LEFT) && KOOGLEMV_CanMove_Simple(self, MOVE_LEFT)){
					ucmd->sidemove = -BOT_SIDE_VEL;
					self->kooglebot.last_strafeDir = MOVE_LEFT;
					self->kooglebot.last_strafeTime = level.framenum + 5;
				}
			}
			//move back to if we can
			if (KOOGLEMV_CanMove(self, MOVE_BACK) && KOOGLEMV_CanMove_Simple(self, MOVE_BACK))
				ucmd->forwardmove = -BOT_FORWARD_VEL;
		}
		else //look away from rocket
		{
			self->client->ps.viewangles[2] = 0;
			VectorSubtract(self->s.origin, self->movetarget->s.origin, self->kooglebot.move_vector);
			KOOGLEMV_ChangeBotAngle(self);

			if (KOOGLEMV_CanMove(self, MOVE_FORWARD) && KOOGLEMV_CanMove_Simple(self, MOVE_FORWARD))
				ucmd->forwardmove = BOT_FORWARD_VEL;
		}
	}
	else
	...
	KOOGLEAI_Reset_Goal_Node(self, 1.0, "Move Dodge Rocket.");
	return true;
}
...
static qboolean KOOGLEMV_MoveDodgeNad(edict_t *self, usercmd_t *ucmd, qboolean targetAquired)
{
	if (!targetAquired)
	{
		...
		KOOGLEMV_ChangeBotAngle(self);
		// strafe left/right
		if (rand() % 2 && KOOGLEMV_CanMove(self, MOVE_FORWARD) && KOOGLEMV_CanMove_Simple(self, MOVE_FORWARD))
		{
			ucmd->forwardmove = BOT_FORWARD_VEL;
			ucmd->sidemove = 0;
		}
		else if (KOOGLEMV_CanMove(self, MOVE_RIGHT) && KOOGLEMV_CanMove(self, MOVE_FORWARD)
			&& KOOGLEMV_CanMove_Simple(self, MOVE_RIGHT) && KOOGLEMV_CanMove_Simple(self, MOVE_FORWARD))
		{
			ucmd->sidemove = BOT_SIDE_VEL;
			ucmd->forwardmove = BOT_FORWARD_VEL;
		}
		else if (KOOGLEMV_CanMove(self, MOVE_LEFT) && KOOGLEMV_CanMove_Simple(self, MOVE_LEFT))
			ucmd->sidemove = -BOT_SIDE_VEL;
	}
	else
	{
		...
		if (angleDif < 0.0 && angleDif > -180)
		{
			if (KOOGLEMV_CanMove(self, MOVE_RIGHT) && KOOGLEMV_CanMove_Simple(self, MOVE_RIGHT)){
				ucmd->sidemove = BOT_SIDE_VEL;
				self->kooglebot.last_strafeDir = MOVE_RIGHT;
				self->kooglebot.last_strafeTime = level.framenum + 10;
			}
		}
		else
		{
	...
	KOOGLEAI_Reset_Goal_Node(self, 2.0, "Move Dodge Nad.");
	return true;
...
{
	static int i=0;
	float skill = bot->kooglebot.botSkillCalculated;

	if (skill >= 3.0f)
		return true;

	if (bot->kooglebot.botSkillDeleyTimer2 > level.framenum){
		if (bot->kooglebot.last_dodgeRocket)
			return true;
		else
			return false;
	}

	bot->kooglebot.botSkillDeleyTimer2 = level.framenum + 40;
	i += 1;

	if (skill >= 2.0f)
	{	// 3/4
		if (i % 4 ||(i+1) % 4 ||(i+2) % 4 ){
			bot->kooglebot.last_dodgeRocket = true;
			return true;
		}
	}
	else if (skill >= 1.0f)
	...
	bot->kooglebot.last_dodgeRocket = false;
	return false;
}
...
///////////////////////////////////////////////////////////////////////
static qboolean KOOGLEMV_MoveToGoal(edict_t *self, usercmd_t *ucmd)
{
	...
	{
		if (KOOGLEMV_Attack_Dodge_bySkill(self))
			KOOGLEMV_MoveDodgeRocket(self, ucmd, false);
		else
		{
			self->client->ps.viewangles[2] = 0;
			VectorSubtract(self->movetarget->s.origin, self->s.origin, self->kooglebot.move_vector);
			KOOGLEMV_ChangeBotAngle(self);

			// strafe left/right
			if (rand() % 2 && KOOGLEMV_CanMove(self, MOVE_LEFT) && KOOGLEMV_CanMove_Simple(self, MOVE_LEFT))
				ucmd->sidemove = -BOT_SIDE_VEL;
			else if (KOOGLEMV_CanMove(self, MOVE_RIGHT) && KOOGLEMV_CanMove_Simple(self, MOVE_RIGHT))
				ucmd->sidemove = BOT_SIDE_VEL;
		}
	...
	{
		if (KOOGLEMV_Attack_Dodge_bySkill(self))
			KOOGLEMV_MoveDodgeNad(self, ucmd, false);
		else
		{
			self->client->ps.viewangles[2] = 0;
			VectorSubtract(self->movetarget->s.origin, self->s.origin, self->kooglebot.move_vector);
			...
			// strafe left/right
			if (rand() % 2 && KOOGLEMV_CanMove(self, MOVE_BACK) && KOOGLEMV_CanMove_Simple(self, MOVE_BACK))
			{
				ucmd->forwardmove = -BOT_FORWARD_VEL;
				ucmd->sidemove = 0;
			}
			else if (KOOGLEMV_CanMove(self, MOVE_RIGHT) && KOOGLEMV_CanMove(self, MOVE_BACK)
				&& KOOGLEMV_CanMove_Simple(self, MOVE_RIGHT) && KOOGLEMV_CanMove_Simple(self, MOVE_BACK))
			{
				ucmd->sidemove = BOT_SIDE_VEL;
				ucmd->forwardmove = -BOT_FORWARD_VEL;
			}
	...
	else if (strcmp(self->movetarget->classname, "trigger_push") == 0)
	{
		...
		self->client->ps.viewangles[2] = 0;
		VectorSubtract(itemOrigin, self->s.origin, self->kooglebot.move_vector);
		KOOGLEMV_ChangeBotAngle(self);
		ucmd->forwardmove = BOT_FORWARD_VEL;
	...
	{	//hypov8 make bot crough if close to cash
		...
		VectorSubtract(self->movetarget->s.origin, self->s.origin, self->kooglebot.move_vector);
		KOOGLEMV_ChangeBotAngle(self);
		if (self->kooglebot.isJumpToCrate)
			ucmd->upmove+= BOT_JUMP_VEL;
		ucmd->forwardmove = BOT_FORWARD_VEL;
		return false;
	}
...
qboolean KOOGLEMV_CheckLadder(edict_t *self, usercmd_t *ucmd, qboolean isTopOfLadder, qboolean isKnownLadder)
{
	...
	if (!isKnownLadder && !self->kooglebot.isOnLadder)
	{
		VectorCopy(self->s.angles, dir);
	...
	if (isKnownLadder || checkDir||self->kooglebot.isOnLadder)
	{
		for (i = 0; i <= 3; i++)
		{
			...
			{ //face ladder
				...
				VectorSubtract(target, self->s.origin, self->kooglebot.move_vector);
				vectoangles(self->kooglebot.move_vector, self->s.angles);
				VectorNormalize(self->kooglebot.move_vector);
				break;
			}
		}
		...
		{
			if (self->groundentity==NULL&& self->kooglebot.ladder_time <= level.framenum 
				&& (!isKnownLadder || self->kooglebot.state == BOTSTATE_WANDER))
			{
				short closest_node = KOOGLEND_FindClosestReachableNode(self, BOTNODE_DENSITY_QUART, BOTNODE_LADDER);
			...
			ucmd->upmove = BOT_JUMP_VEL;
			//ucmd->forwardmove = 0;
			self->kooglebot.ladder_time = level.framenum + 2; //hypov8 was 3
			self->kooglebot.isOnLadder = true; //hypo stop bot attacking on ladders
			if (isTopOfLadder)
				self->kooglebot.isOnLadder = 2;
			return true;
		}
	}
...
{
	//bot used jump pad.
	if (bot->kooglebot.is_bot)
	{
		//hypov8 todo: wander/goal. make jump pads a goal?
		bot->kooglebot.trigPushTimer = level.framenum + 5; //0.5 seconds untill bot can move
		bot->kooglebot.isMovingUpPushed = true;
		...
		bot->kooglebot.node_current = INVALID;
		bot->kooglebot.node_next = INVALID;
		bot->kooglebot.node_goal = INVALID;
		bot->kooglebot.node_ent = NULL;

		bot->kooglebot.isTrigPush = true;
		VectorCopy(bot->s.origin, bot->kooglebot.oldOrigin);
		bot->kooglebot.targetPlayerNode = 0;
		bot->kooglebot.wander_timeout = level.time + 0.2;
...
void KOOGLEMV_Move(edict_t *self, usercmd_t *ucmd)
{
	...
	//add hypov8
	if (self->kooglebot.isTrigPush && self->groundentity) //todo:
		self->kooglebot.isTrigPush = false;
	...
	{
		self->kooglebot.state = BOTSTATE_WANDER;
		self->kooglebot.targetPlayerNode = 0;
		self->kooglebot.wander_timeout = level.time + 1.0;
		return;
	}

	current_node_type = nodes[self->kooglebot.node_current].type;
	next_node_type = nodes[self->kooglebot.node_next].type;
	...
	if(next_node_type == BOTNODE_GRAPPLE)
	{
		if (!(self->client->hookstate & HOOK_ON))
		{
			...
			VectorSubtract(nodes[self->kooglebot.node_next].origin, vHeight, self->kooglebot.move_vector);
			vectoangles(self->kooglebot.move_vector, self->s.angles);
			...
			self->kooglebot.hookDistLast = hook_max_length->value;
			self->kooglebot.hookDistCurrent = hook_max_length->value-1;
			FireHook(self);
		}
		else
			KOOGLEMV_ChangeBotAngle(self);
	...
	if (current_node_type != BOTNODE_PLATFORM && next_node_type == BOTNODE_PLATFORM)
	{
		...
		for (i = 0; i < num_items; i++)
			if (item_table[i].node == self->kooglebot.node_next)
				if (item_table[i].ent->moveinfo.state != STATE_BOTTOM)
				{
					if (self->kooglebot.plateWaitTim >= level.framenum && self->kooglebot.plateWaitTim <= level.framenum + 10)
					{
						self->kooglebot.state = BOTSTATE_WANDER;
						self->kooglebot.wander_timeout = level.time + 2.0;
						return;
					}
					if (self->kooglebot.plateWaitTim < level.framenum)

						self->kooglebot.plateWaitTim = level.framenum + 70;
	...
	{
		// Move to the center
		self->kooglebot.move_vector[2] = 0; // kill z movement	
		if (VectorLength(self->kooglebot.move_vector) > 10)
	...
	{
		// Set up a jump move
		if (self->kooglebot.moveDirVel > 2) //add hypov8. bot can be blocked in mid air. //&& self->groundentity == NULL
		{
			if (!self->kooglebot.isTrigPush) //todo:
			{
				float dist = VectorDistanceFlat(nodes[self->kooglebot.node_next].origin, self->s.origin);
				float height = nodes[self->kooglebot.node_next].origin[2] - self->s.origin[2] + 4;
				VectorSubtract(nodes[self->kooglebot.node_next].origin, self->s.origin, self->kooglebot.move_vector);
				self->kooglebot.move_vector[2] = 0.0f; //look straight
				VectorNormalize(self->kooglebot.move_vector);
				...
				if (self->groundentity)
				{
					...
					self->velocity[0] = self->kooglebot.move_vector[0] * 360*dist;
					self->velocity[1] = self->kooglebot.move_vector[1] * 360*dist;
					self->groundentity = NULL;
					ucmd->upmove = BOT_JUMP_VEL/2;
					self->kooglebot.is_Jumping = true;				
				}
				else // traveling in air to goal.
				{
					...
					//else 
					{
						/*if (next_node_type == BOTNODE_JUMP)
						{
							//dist = 32;
							dist /= 360.0f;
							self->velocity[0] = self->kooglebot.move_vector[0] * (360 * dist); //hypov8 todo. scale??
							self->velocity[1] = self->kooglebot.move_vector[1] * (360 * dist);
						}*/
					}

						
					if (next_node_type == BOTNODE_JUMP 	&& self->s.origin[2] + 60 < nodes[self->kooglebot.node_next].origin[2])
						KOOGLEAI_Reset_Goal_Node(self, 0.1, "Could not reach jump node");
				}
		...
		else
		{
			...
			KOOGLEAI_Reset_Goal_Node(self, 1.0, "Bot Stuck in the air.");
	...
	if (next_node_type == BOTNODE_LADDER && nodes[self->kooglebot.node_next].origin[2] > self->s.origin[2])
	{
		if (current_node_type == BOTNODE_LADDER)
		{
			if (self->kooglebot.node_next != self->kooglebot.node_current							//make sure its a differnt node
				&& self->s.origin[2] > nodes[self->kooglebot.node_current].origin[2]			//make sure we are higher then last node
				&& nodes[self->kooglebot.node_next].origin[2] > nodes[self->kooglebot.node_current].origin[2] )	//make sure next node is above
			{
				...
				heightTOTAL = nodes[self->kooglebot.node_next].origin[2] - nodes[self->kooglebot.node_current].origin[2];
				heightUP = heightTOTAL - (nodes[self->kooglebot.node_next].origin[2] - (self->s.origin[2]+ BOTNODE_SHIFT));
				heightMOVE =  ((1/ heightTOTAL)*heightUP);

				VectorSubtract(nodes[self->kooglebot.node_next].origin, nodes[self->kooglebot.node_current].origin, moveDist);
				VectorMA(nodes[self->kooglebot.node_current].origin, heightMOVE, moveDist, moveTO);
		...
		if (KOOGLEMV_CheckLadder(self, ucmd, false, true))
			return;
		else
		{
			...
			float distToTarget;
			//look at next node, flat
			VectorCopy(nodes[self->kooglebot.node_next].origin, dist);
			dist[2] = self->s.origin[2];
			VectorSubtract(dist, self->s.origin, self->kooglebot.move_vector);
			...
			VectorSubtract(nodes[self->kooglebot.node_next].origin, self->s.origin, self->kooglebot.move_vector);
			vectoangles(self->kooglebot.move_vector, self->s.angles);//angles
			VectorNormalize(self->kooglebot.move_vector);
			//KOOGLEMV_ChangeBotAngle(self);
			return;
#endif
	...
	if(current_node_type == BOTNODE_LADDER && next_node_type != BOTNODE_LADDER /*&&
		nodes[self->kooglebot.node_next].origin[2] > self->s.origin[2]-8*/) //move player origin down// increase??
	{
		if (!KOOGLEMV_CheckLadder(self, ucmd,true,true))
		{
			// keep moving up
			if (self->kooglebot.isOnLadder)
			{
				ucmd->forwardmove = BOT_FORWARD_VEL;
				ucmd->upmove = 50;
			}
	...
	if(current_node_type == BOTNODE_WATER)
	{
		...
		if (next_node_type != BOTNODE_WATER && !(gi.pointcontents(nodes[self->kooglebot.node_next].origin) & MASK_WATER)) // Exit water
			ucmd->upmove = BOT_JUMP_VEL;
	...
	if (!self->groundentity && !self->kooglebot.isMovingUpPushed)
	{

		if (!KOOGLEMV_CheckLadder(self, ucmd, false, false))
		{
			if (self->kooglebot.node_next != INVALID)
				VectorSubtract(nodes[self->kooglebot.node_next].origin, self->s.origin, self->kooglebot.move_vector);

			KOOGLEMV_ChangeBotAngle(self);

			if (!self->kooglebot.is_Jumping)//falling to jump node. didnt touch ground yet
			{
				//begin fall to close node. slow player to fit inside tight gaps
				float scale = 1.0f;
				float distPlayer = VectorDistanceFlat(nodes[self->kooglebot.node_next].origin, self->s.origin);
				...
				/*if (level.bot_mapFix)
				{
					...
					{
						float height = nodes[self->kooglebot.node_next].origin[2] - self->s.origin[2];
						
						float distNode = VectorDistanceFlat(nodes[self->kooglebot.node_next].origin, nodes[self->kooglebot.node_current].origin);
				...
				//ucmd->forwardmove = BOT_FORWARD_VEL;
				self->velocity[0] = self->kooglebot.move_vector[0] * (360* scale); //hypov8 todo. scale??
				self->velocity[1] = self->kooglebot.move_vector[1] * (360* scale);
			...
			else
			{
				...
				{
					VectorSubtract(nodes[self->kooglebot.node_next].origin, self->s.origin, self->kooglebot.move_vector);
					self->kooglebot.move_vector[2] = 0;
					vectoangles(self->kooglebot.move_vector, self->s.angles);
					VectorCopy(self->s.angles, self->client->v_angle);
				}
				ucmd->forwardmove = BOT_FORWARD_VEL;
	...
	if (self->kooglebot.moveDirVel < 5 && !self->kooglebot.isMovingUpPushed) //hypov8 todo: was 15!!!!!
	//if (VectorLength(self->velocity) < 37 && !self->kooglebot.isMovingUpPushed)
	{
	...
	if (self->kooglebot.isMovingUpPushed && next_node_type == BOTNODE_MOVE)
	{
		vec3_t angles;
		//self->client->ps.viewangles[2] = 0;
		//VectorSubtract(nodes[self->kooglebot.node_next].origin, self->s.origin, self->kooglebot.move_vector);
		...
		VectorSubtract(nodes[self->kooglebot.node_next].origin, self->s.origin, self->kooglebot.move_vector);
		vectoangles(self->kooglebot.move_vector, angles);
...
void KOOGLEMV_WanderPlat(edict_t *self, usercmd_t *ucmd, edict_t *plate)
{
	...
	if (distToTarget <= 16 && self->s.origin[2] > self->kooglebot.oldOrigin[2] + 2)
	{
		self->velocity[0] = 0;
		self->velocity[1] = 0;
		self->velocity[2] = 0;
		self->kooglebot.next_move_time = level.time + 0.5;
		return;
	}
	else
	{
		vec3_t angles;
		VectorSubtract(vCenter, self->s.origin, self->kooglebot.move_vector);
		vectoangles(self->kooglebot.move_vector, angles);
		VectorCopy(angles, self->s.angles);
		ucmd->forwardmove = BOT_FORWARD_VEL *0.75;
		return;
...
void KOOGLEMV_Wander(edict_t *self, usercmd_t *ucmd)
{
	...
	// Do not move
	if (self->kooglebot.next_move_time > level.time)
	...
	/*if (self->kooglebot.isJumpToCrate) {
		if (level.framenum > self->kooglebot.crate_time)
			self->kooglebot.isJumpToCrate = false;	}*/
	...
	if (Q_stricmp(trace.ent->classname, "func_plat")== 0)
	{
		...
		{
			KOOGLEMV_WanderPlat(self, ucmd, trace.ent);
			return;
		}
	}
	...
	if (self->groundentity != NULL && self->groundentity->use == Use_Plat)
	{
		...
		{
			KOOGLEMV_WanderPlat(self, ucmd, self->groundentity);
			return;
		}
	}
	...
	if(gi.pointcontents (temp) & MASK_WATER)
	{
		...
		else
		{
			if (self->goalentity  && self->goalentity->solid != SOLID_NOT)
			{
				int index;
				float weight;
				index = KOOGLEIT_ClassnameToIndex(self->goalentity->classname, self->style); //hypov8 add safe styles
				weight = KOOGLEIT_ItemNeed(self, index, 0.0, self->goalentity->spawnflags);

				if (weight > 0.0f)
				{
					vec3_t angles;
					// Set direction
					VectorSubtract(self->goalentity->s.origin, self->s.origin, self->kooglebot.move_vector);
					vectoangles(self->kooglebot.move_vector, angles);
					VectorCopy(angles, self->s.angles);
					ucmd->forwardmove = BOT_FORWARD_VEL *0.75;
	...
	if (!self->kooglebot.isMovingUpPushed)
	{
		if (KOOGLEMV_CheckEyes_Wander(self, ucmd))
			return;
	}
	...
	if (self->groundentity == NULL && self->velocity[2] < -100)
	{
		...
		// Get current angle and set up "eyes"
		VectorSubtract(self->s.origin, self->kooglebot.oldOrigin, dir);
		vectoangles(dir, dir);
		//VectorCopy(self->s.angles, dir);
		AngleVectors(dir, forward, right, NULL);
	...
	if (self->kooglebot.moveDirVel < 5 && !self->kooglebot.isMovingUpPushed) //hypov8 todo was 2. 5ok? 15!!!!
	//if (VectorLength(self->velocity) < 37 && !self->kooglebot.isMovingUpPushed) //hypov8 jump pad
	{
		//vec3_t start;
		if(random() > 0.1 && KOOGLEMV_SpecialMove(self,ucmd))
			return;
	...
	if (self->kooglebot.isMovingUpPushed)
	{
		...
		VectorCopy(self->kooglebot.oldOrigin, tmp2);
		tmp2[2] = self->s.origin[2];

		VectorSubtract(self->s.origin, tmp2, self->kooglebot.move_vector);
		vectoangles(self->kooglebot.move_vector, angles);
		VectorCopy(angles, self->s.angles);
	}
	...
	if (self->client->pers.weapon)
	{
		if ((Q_stricmp(self->client->pers.weapon->classname, "weapon_bazooka") == 0))
		{
			...
			if (range >= 300)
			{
				...
				trace_t trace;
				float skill = self->kooglebot.botSkillCalculated * .25; //0-1
...
static void KOOGLEMV_Attack_AimRandom(edict_t *self)
{
	...
	float skill = (1 - (self->kooglebot.botSkillCalculated *.25));
	...
	{
		if (self->kooglebot.enemyID > 0)
		{
			enemy = g_edicts + self->kooglebot.enemyID;
			hunt = enemy->kooglebot.is_hunted;
		...
		hunt = self->enemy->kooglebot.is_hunted;
		VectorCopy(self->enemy->velocity, velTmp);
	}


	if (hunt && (int)sv_bot_hunt->value)
		self->kooglebot.bot_accuracy = 0; //no randomness for top players
	else
	{
		...
		// Get distance.
		range = VectorDistance(self->kooglebot.enemyOrigin, self->s.origin);
		...
		self->kooglebot.bot_accuracy = rand_y * disAcc;
	}
}
...
void KOOGLEMV_Attack_AimRandom_offset(edict_t *self, vec3_t aimdir)
{
	...
	if (self->kooglebot.enemyID < 1)//posibly dead?
		return;
	...
	//VectorCopy( self->kooglebot.enemyOrigin, enemyOrigin);buggy!!! delayd by 1 frame

	//predict rockets
	KOOGLEMV_Attack_RL_Predict(self, enemy);
	VectorAdd(enemyOrigin, self->kooglebot.enemyRL_Offset, enemyOrigin);

	//offset rocket height. legs..
	enemyOrigin[2] += self->kooglebot.enemyOriginZoffset;
	//enemyOrigin[2] -= 8; //move down some for recoil?

	VectorSubtract(enemyOrigin, botOrigin, v2);
	vectoangles(v2, v);

	v[YAW] += self->kooglebot.bot_accuracy;
	if (v[YAW] < 0)		
		v[YAW] += 360;
	else if	(v[YAW] > 360)		
		v[YAW] -= 360;
...
{
	//random aim
	KOOGLEMV_Attack_AimRandom(self);
...
static void KOOGLEMV_Attack_Dodge(edict_t *self, usercmd_t *ucmd)
{
	...
	float skill = self->kooglebot.botSkillCalculated;
	qboolean is_hunted = self->enemy->kooglebot.is_hunted;
	
	// must still have pistol
	if (!(int)sv_hitmen->value && 
		(self->kooglebot.num_weps <= 2 || (self->client->pers.weapon && !strcmp(self->client->pers.weapon->classname, "weapon_pistol"))) )
	{
		if (self->movetarget && strncmp(self->movetarget->classname, "weapon_", 7) == 0 && self->enemy)
		{
...
#if HYPODEBUG
			if (!self->enemy->kooglebot.is_bot)
			...
			{
				if (KOOGLEMV_CanMove_Simple(self, MOVE_FORWARD))
					ucmd->forwardmove = BOT_FORWARD_VEL;
				else if (KOOGLEMV_CanMove_Simple(self, MOVE_BACK))
					ucmd->forwardmove = -BOT_FORWARD_VEL;
			}//back
			...
			{
				if (KOOGLEMV_CanMove_Simple(self, MOVE_BACK))
					ucmd->forwardmove = -BOT_FORWARD_VEL;
				else if (KOOGLEMV_CanMove_Simple(self, MOVE_FORWARD))
					ucmd->forwardmove = BOT_FORWARD_VEL;
			}
			...
			{
				if (KOOGLEMV_CanMove_Simple(self, MOVE_RIGHT))
					ucmd->sidemove = BOT_SIDE_VEL;
				else if (KOOGLEMV_CanMove_Simple(self, MOVE_LEFT))
					ucmd->sidemove = -BOT_SIDE_VEL;
			}
			return;
		}
	...
	if (skill <= 3 && ! is_hunted)	
	{
		...
		if (c > .2 && skill < c)
		{
			if (self->enemy)
			{
				...
				dist = VectorLength(v);
				if (dist <180 || self->kooglebot.last_moveFwdTime > level.framenum)
				{
					if (KOOGLEMV_CanMove_Simple(self, MOVE_BACK))
						ucmd->forwardmove -= BOT_FORWARD_VEL;
					else if (KOOGLEMV_CanMove_Simple(self, MOVE_FORWARD))
						ucmd->forwardmove += BOT_FORWARD_VEL;
					if ((self->kooglebot.last_moveFwdTime + 25) < level.framenum)
					self->kooglebot.last_moveFwdTime = level.framenum + 25;
				}
				else
				{
					if (KOOGLEMV_CanMove_Simple(self, MOVE_FORWARD))
						ucmd->forwardmove += BOT_FORWARD_VEL;
					self->kooglebot.last_moveFwdTime = 0;
				}
			}
			return;
		}
...
#if 1
	if (self->movetarget && self->movetarget->owner == self->enemy && ( is_hunted|| KOOGLEMV_Attack_Dodge_bySkill(self)))
	{
		if (strcmp(self->movetarget->classname, "rocket") == 0)
		{
			KOOGLEMV_MoveDodgeRocket(self, ucmd, true);
			return;
		}
		else if (strcmp(self->movetarget->classname, "grenade") == 0)
		{
			KOOGLEMV_MoveDodgeNad(self, ucmd, true);
			return;
		}
	}
	...
	if (self->kooglebot.last_strafeTime > level.framenum)
	{
		strafe = true;
		if (self->kooglebot.last_strafeDir == MOVE_LEFT)
			strafeDir = MOVE_LEFT;
		else if (self->kooglebot.last_strafeDir == MOVE_RIGHT)
	...
	if (!self->kooglebot.isMovingUpPushed)
	{
		...
		//Com_Printf("strafeDir=%i, strafe=%i, rand=%d\n", self->kooglebot.last_strafeDir, strafe, c);

		if (((c < 0.500f && !strafe) || (strafe && strafeDir == MOVE_LEFT))
			&& KOOGLEMV_CanMove(self, MOVE_LEFT) && KOOGLEMV_CanMove_Simple(self, MOVE_LEFT)){
			ucmd->sidemove -= BOT_SIDE_VEL;
			moveResolved = true;
			if (!strafe){
				self->kooglebot.last_strafeTime = level.framenum + frames;
				self->kooglebot.last_strafeDir = MOVE_LEFT;
			}
		}
		else if (((c >= 0.500f && !strafe) || (strafe && strafeDir == MOVE_RIGHT))
			&& KOOGLEMV_CanMove(self, MOVE_RIGHT) && KOOGLEMV_CanMove_Simple(self, MOVE_RIGHT)){
			ucmd->sidemove += BOT_SIDE_VEL;
			moveResolved = true;
			if (!strafe){
				self->kooglebot.last_strafeTime = level.framenum + frames;
		...
		if (c < 0.3 && KOOGLEMV_CanMove(self, MOVE_FORWARD) && KOOGLEMV_CanMove_Simple(self, MOVE_FORWARD)){
			ucmd->forwardmove += BOT_FORWARD_VEL;
			moveResolved = true;
		}
		else if (c > 0.7 && KOOGLEMV_CanMove(self, MOVE_BACK) && KOOGLEMV_CanMove_Simple(self, MOVE_BACK)){ //was forward??
			ucmd->forwardmove -= BOT_FORWARD_VEL;
			moveResolved = true;
		}
	}
	...
	if (!moveResolved && !self->kooglebot.isMovingUpPushed && self->groundentity)
		if (c < 0.500f)
		{
			if (KOOGLEMV_CanMove_Simple(self, MOVE_LEFT)){
				ucmd->sidemove -= BOT_SIDE_VEL;
...
void KOOGLEMV_Attack (edict_t *self, usercmd_t *ucmd)
{
	...
	//hypov8 taunt
	if (self->kooglebot.tauntTime < level.framenum)
	{
		float c = random();
		float skill = 1-(self->kooglebot.botSkillCalculated * .25); //1-0
		KOOGLEMV_BotTaunt(self, self->enemy);
		self->kooglebot.tauntTime = level.framenum + (100+ (c *skill * 300)); 
	}


	//on hook while attacking
	if (self->client->hookstate != 0 &&
		self->kooglebot.node_next != INVALID && nodes[self->kooglebot.node_next].type == BOTNODE_GRAPPLE)
	{
		vec3_t to,v;
		// Are we there yet?
		VectorCopy(self->s.origin, to);
		to[2] += 8; //shift up!
		VectorSubtract(to, nodes[self->kooglebot.node_next].origin, v);
	...
	if (self->client->pers.weapon)
	{
		if ((Q_stricmp(self->client->pers.weapon->classname, "weapon_bazooka") == 0))
		{
			if (self->kooglebot.aimLegs)	{
				boozooka = true;
				target[2] -= 32; //hypov8 move bots aim at feet (72 kp)
				self->kooglebot.enemyOriginZoffset = -32;
			}
			...
			//set aim hieght
			//predict aim ahead
			//KOOGLEMV_Attack_RL_Predict(self, self->enemy);
			//VectorAdd(target, self->kooglebot.enemyRL_Offset, target);
		...
		else if ((Q_stricmp(self->client->pers.weapon->classname, "weapon_grenadelauncher") == 0))
		{
			...
			target[2] += offset;
			self->kooglebot.enemyOriginZoffset = offset;

			grenad = true;
		}
		else if ((Q_stricmp(self->client->pers.weapon->classname, "weapon_flamethrower") == 0))
		{
			if( self->kooglebot.botSkillCalculated < 3.2f)
				flameGun=true;
		}
	...
	if (!boozooka && !grenad)
	{
		if ((self->enemy->client->ps.pmove.pm_flags & PMF_DUCKED) && !self->kooglebot.aimLegs){
			target[2] -= 28; //hypov8 move bots aim down todo: check fence???
		self->kooglebot.enemyOriginZoffset = -28;
	}
		else if (self->kooglebot.aimLegs){
			target[2] -= 36; //player obstructed with something above legs
			self->kooglebot.enemyOriginZoffset = -36;
		}
	}


	//set enemy origin for bullet calculation. includes look up/down
	VectorCopy(target, self->kooglebot.enemyOrigin);

	if (flameGun)
	{
		//vec3_t fDir;
		if (self->kooglebot.flame_frameNum < level.framenum)
		{
			self->kooglebot.flame_frameNum = level.framenum + 20;
			//KOOGLEMV_Attack_CalcRandDir(self, fDir);
			KOOGLEMV_Attack_AimRandom(self);
		}

		VectorSubtract(self->kooglebot.enemyOrigin, self->s.origin, self->kooglebot.move_vector);
		KOOGLEMV_Attack_AimRandom_offset(self, self->kooglebot.move_vector);
		KOOGLEMV_ChangeBotAngle(self);
	}
	else
	{
	...
	//setup dodge
	KOOGLEMV_Attack_Dodge(self, ucmd); //hypov8 moved down here
	...
	if (self->client->hookstate == 0 &&	self->kooglebot.state == BOTSTATE_MOVE) //if not on hook
	{
		//add hypov8. try to stop bots being jerky after target is dead/lost
		if (self->kooglebot.isChasingEnemy)
			KOOGLEAI_Reset_Goal_Node(self, 1.0, "Found Enemy. Stop Chasing");
		else
			KOOGLEAI_Reset_Goal_Node(self, 1.0, "");//"Found Enemy."
	}

	self->kooglebot.isChasingEnemy = false;
	self->kooglebot.enemyChaseFrame = 0;
