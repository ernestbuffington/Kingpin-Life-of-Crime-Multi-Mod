	...
	while (fgetline(pIn, buffer))
	{
		...
		if (valid)
		{
			...
			if (teamplay->value) // name, skin, team 
				KOOGLESP_SpawnBot(player.team, player.name, player.skin, NULL, player.skill); //sv addbot thugBot "male_thug/009 031 031" dragon
			else // name, skin			
				KOOGLESP_SpawnBot("\0", player.name, player.skin, NULL, player.skill); //sv addbot thugBot "male_thug/009 031 031"
		}
	}
...
static void KOOGLESP_HoldSpawn(edict_t *self)
{
	...
	if (teamplay->value)
		KOOGLESP_PutClientInServer(self, true, self->client->pers.team);
	else
...
static void KOOGLESP_ClientConnect(edict_t *ent, char *userinfo)
{
	...
	switch (ent->kooglebot.randomWeapon)
	{
...
static  void KOOGLESP_PutClientInServer(edict_t *bot, qboolean respawn, int team)
{
	...
	bot->enemy = NULL;
	bot->movetarget = NULL;
	bot->kooglebot.state = BOTSTATE_MOVE;
	bot->kooglebot.suicide_timeout = level.time + 15.0;

	// Set the current node
	bot->kooglebot.node_ent = NULL;
	bot->kooglebot.next_move_time = level.time;
	bot->kooglebot.node_current = KOOGLEND_FindClosestReachableNode(bot, BOTNODE_DENSITY, BOTNODE_ALL);
	bot->kooglebot.node_goal = bot->kooglebot.node_current;
	bot->kooglebot.node_next = bot->kooglebot.node_current;
	if (bot->kooglebot.node_current == INVALID) 
...
//		skins = strstr( s, "/" ) + 1;
...

	{
		...
		if (respawn)
		{
			...
			bot->kooglebot.node_goal = bot->kooglebot.node_current;
			bot->kooglebot.node_next = bot->kooglebot.node_current;
			if (bot->kooglebot.node_current == INVALID) bot->kooglebot.state = BOTSTATE_WANDER;
// BEGIN HITMEN
			if (!sv_hitmen->value)
//END
				if (!KOOGLEAI_PickShortRangeGoalSpawned(bot))
		...
		if (!respawn)
		{
			bot->think = KOOGLESP_HoldSpawn; //hypov8 ToDo: causing telfrag of waiting at spawn?
...
void KOOGLESP_Respawn (edict_t *ent)
{
	...
	{
		ent->kooglebot.isTrigPush = false;
		ent->kooglebot.enemyID = -1; //hypo add
		ent->kooglebot.num_weps = 2;  //hypo add. 2= pistol+pipe
		ent->kooglebot.lastDamageTimer = 0;  //hypo add
		ent->kooglebot.enemyAddFrame = 0;	//hypov8 add
		ent->kooglebot.tauntTime = level.framenum + (random() * 100);
		//ent->kooglebot.spawnedTime = level.framenum + 10; 
		ent->kooglebot.SRGoal_frameNum = 0;
		ent->enemy = NULL;
		ent->movetarget = NULL;
		ent->kooglebot.state = BOTSTATE_MOVE;
		ent->kooglebot.suicide_timeout = level.time + 15.0;

		// Set the current node
		ent->kooglebot.node_ent = NULL;
		ent->kooglebot.next_move_time = level.time;
		ent->kooglebot.node_current = KOOGLEND_FindClosestReachableNode(ent, BOTNODE_DENSITY, BOTNODE_ALL);
		ent->kooglebot.node_goal = ent->kooglebot.node_current;
		ent->kooglebot.node_next = ent->kooglebot.node_current;
		if (ent->kooglebot.node_current == INVALID) 
			ent->kooglebot.state = BOTSTATE_WANDER;

		// BEGIN HITMEN
		if (sv_hitmen->value) //todo: check this
			KOOGLEAI_PickLongRangeGoal(ent);//todo: lrg weapon
		else
		//END
			if (!KOOGLEAI_PickShortRangeGoalSpawned(ent))
				KOOGLEAI_PickLongRangeGoal(ent);//todo: lrg weapon
	}
...
void KOOGLESP_SetName(edict_t *bot, char *name, char *skin/*, char *team*/)
{
...
#if 0
	{
		...
		gi.dprintf("KOOGLE: bot (%s) connected\n", bot_name);
...
void KOOGLESP_SpawnBot (char *team, char *name, char *skin, char *userinfo, float skill)
{
	...
	bot->kooglebot.enemyID = -1;
	bot->yaw_speed = 100; // yaw speed
	bot->inuse = true;
	bot->kooglebot.is_bot = true;
	bot->kooglebot.botSkillMultiplier = skill;
	...
	KOOGLESP_PutClientInServer(bot, false, bot->client->pers.team /*TEAM_NONE*/);
