...
void DropHook (edict_t *ent) {
  ...
  // acebot
  if (ent->owner->kooglebot.pm_hookActive)
	ent->owner->kooglebot.pm_last_node = INVALID;

  ent->owner->kooglebot.hookDistLast = 0;
  ent->owner->kooglebot.hookDistCurrent= 0;
  ent->owner->kooglebot.pm_hookActive = 0;
  //end
...
void HookBehavior(edict_t *ent) {
  ...
  // grow the length of the chain
	...
	ent->owner->kooglebot.hookDistLast = ent->owner->kooglebot.hookDistCurrent;
	if (ent->owner->kooglebot.is_bot)
	{
		if ((ent->owner->client->hookstate & SHRINK_ON) && (ent->angle > 2.0))
		{
			ent->angle -= hook_pull_speed->value;
			if (ent->angle < 2.0) ent->angle = 2.0;
			ent->owner->kooglebot.hookDistCurrent = ent->angle;
